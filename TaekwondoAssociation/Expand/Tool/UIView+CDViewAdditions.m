...
- (void)showView:(UIView *)view WithBackViewAlpha:(CGFloat)a Target:(id)target TouchAction:(SEL)selector{
...
- (void)showView:(UIView *)view WithBackViewAlpha:(CGFloat)a Target:(id)target TouchAction:(SEL)selector Animation:(void(^)(void))animation TimeInterval:(NSTimeInterval)interval Fininshed:(void(^)(BOOL finished))fininshed{
    [self showView:view WithBackViewAlpha:a Target:target TouchAction:selector];
...
- (BOOL)includeSubviewInHierarchy:(UIView *)view {
    for (UIView *subview in self.subviews) {
        if (subview == view) {
            return YES;
        }
        if ([subview includeSubviewInHierarchy:view]) {
            return YES;
        }
    }
    return NO;
...
- (BOOL)isIncludedInSuperviewHierarchy:(UIView *)view {
    return [view includeSubviewInHierarchy:self];
}
...
- (UIResponder *)getFirstResponder {
    for (UIView *subview in self.subviews) {
        ...
        UIResponder *responder = [subview getFirstResponder];
        if (responder) {
            return responder;
        }
    }
...
- (UITextField *)getFirstResponderTextField {
    UIResponder *textField = [self getFirstResponder];
