...
//    [TalkingData trackPageBegin:@"JKImagePicker_Controller"];
//    [MobClick beginLogPageView:@"JKImagePicker_Controller"];
}
...
//    [TalkingData trackPageEnd:@"JKImagePicker_Controller"];
//    [MobClick endLogPageView:@"JKImagePicker_Controller"];
...
- (void)setUpProperties
{
    ...
    [preBtn setTitle:@"预览" forState:UIControlStateNormal];
       [preBtn.titleLabel setFont:[UIFont systemFontOfSize:15]];
...
//    [preBtn setTitleColor:[UIColor colorWithType:StandardColorTypeWhite alpha:0.6] forState:UIControlStateDisabled];
                             ...
                             if ([assetsGroups count]>0) {
                                 ...
                                 for(ALAssetsGroup *group in assetsGroups)
                                 {
                                     if ([[group valueForProperty:ALAssetsGroupPropertyName] isEqualToString:@"相机胶卷"]) {
                                         index = [assetsGroups indexOfObject:group];
                                     }
                                 }
...
{    
    ...
    if ([indexPath row]<=0) {
        ...
        NSInteger numberOfAssets = self.assetsArray.count;
        
        ALAsset *asset = self.assetsArray[numberOfAssets-indexPath.row];
        cell.asset = asset;
...
{
      NSInteger numberOfAssets = self.assetsArray.count;
    [self browerPhotoes:self.assetsArray page:numberOfAssets-indexPath.row];
}
...
- (void)resetFinishFrame
{
    ...
    self.finishLabel.text = [NSString stringWithFormat:@"确定(%ld)",(unsigned long)self.selectedAssetArray.count];
